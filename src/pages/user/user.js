...
export default class User extends React.Component {
  ...
  handleOperator = (type) => {
    ...
    if (type === 'create') {
      ...
      if (!item) {
        Modal.info({ title: '信息', content: '请选择一个用户' })
        return
      }
      Modal.confirm({
        text: '确定要删除此用户吗？',
    ...
    return (
      <div>
        ...
        <div className="content-wrap">
          <ETable columns={columns} updateSelectedItem={Utils.updateSelectedItem.bind(this)} selectedRowKeys={this.state.selectedRowKeys} dataSource={this.state.list} pagination={this.state.pagination}/>
        </div>
        <Modal title={this.state.title} visible={this.state.isVisible} onOk={this.handleSubmit} width={800} onCancel={()=>{
